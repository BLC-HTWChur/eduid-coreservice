...
class TokenDataValidator extends EduIDValidator {
    ...
    protected function validate() {
        ...
        if (isset($this->data) && !empty($this->data)) {
            ...
            else if ($this->data["grant_type"] == "password") {
            ...
            else if ($this->data["grant_type"] == "authorization_code") {
                $token = $this->service->getAuthToken();

                if ($token["authorization_key"] != $this->data["code"]) {
                    $this->log("mismatching code presented");
                    return false;
                }

                if ($token["extras"]["client_type"] != $this->data["client_id"]) {
